...
typedef struct{
  ...
  bool itemMas; //True == se Añadió un ítem; False == Se eliminó un ítem
  char item[100];
  int ph;
}TipoAccion;
...
void insertarJugador(List *jugadores){
  ...
  printf("===============================================================");
  printf("\n            Se registró exitosamente al jugador\n");
  printf("===============================================================\n");
}
...
void insertaItem(TipoJugador *jugador, Map *mapaItems, bool archivo){
  ...
  if(archivo==false){
    ...
    jugador->cantItems++;
    printf("===============================================================");
    printf("\n              Se agregó correctamente el ítem\n");
    printf("===============================================================\n");
  }
  else{
  ...
  List* itemActual = searchMap(mapaItems,item);
...
void ItemEspecifico(List *jugadores,Map *mapaItems){
  char item[100];
  solicitarString(item,"Ingrese el nombre del item:");
...
void deshacerAccion(List *jugadores,Map *mapaItems){
  ...
  if(accion==NULL){
    printf("⚠⚠⚠ El usuario no ha realizado acciones con este jugador ⚠⚠⚠\n");
    return;
  }
//Si el booliano de la acción es verdadera quiere decir que tiene que ver con que la última acción fue insertar o eliminar item, sino es que se agregaron puntos de habilidad
if(accion->accion==true){
    //Si itemMas es true quiere decir que la ultima acción fue agregar un ítem, por lo que ahora hay que eliminarlo, si es falso quiere decir que se eliminó un ítem, por lo que hay que volver a agregarlo
    if(accion->itemMas == true){
      jugador->cantItems--;
      
      //Se elimina el item de la lista de items y la última acción de la pila de acciones
      for (char *item = firstList(jugador->items) ; item != NULL ; item = nextList(jugador->items)){
        if(strcmp(item,accion->item)==0){
          popCurrent(jugador->items);
          printf("╔═════════════════════════════════════════════════════════════╗\n");
          printf("║     El ítem ha sido eliminado correctamente del jugador     ║\n");
          printf("╚═════════════════════════════════════════════════════════════╝\n");
        }
      }
      
      popFront(jugador->pilaAcc);
      //Se toma la lista de jugadores del mapa de items para restarle uno al contador de jugador correspondiente al item, en caso 
...
void menu(List *jugadores,Map*mapaItems){
  ...
  while(opcion != 0){
    ...
    switch(opcion){
      ...
      case 9: importarArchivo(mapaItems,jugadores);
      break;
